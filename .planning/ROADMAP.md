...
- [ ] **Phase 2: Core Data API** - All CRUD endpoints ported with JWT auth enforcement (lessons, clusters, KBs, teachers, admin, settings)
- [ ] **Phase 3: Adaptation Engine** - RAG pipeline, OpenRouter LLM, lesson adaptation+versioning, source file editing (DOCX/PPTX/PDF)
...
Plans:
- [ ] 02-01: Lessons CRUD, Clusters CRUD, Knowledge Bases endpoints with JWT auth
- [ ] 02-02: Teachers dashboard, classes, student cluster assignment endpoints with RBAC
- [ ] 02-03: LLM config CRUD, API key encryption, connection test endpoint, Admin overview endpoints
...
Plans:
- [ ] 03-01: RAG pipeline (embedder, ChromaDB store, chunker, retriever) + OpenRouter LLM provider adapter
- [ ] 03-02: Adaptation service (generate, refine, feedback) + versioning service (head pointer, rollback, transactions) + EJS rendering
- [ ] 03-03: Source file editing endpoints (DOCX, PPTX, PDF extraction + LLM editing + download)
...
Plans:
- [ ] 04-01: Vite + React + Router setup, AuthContext, useApi hook, Login, Register, First-login pages, protected route wrapper, shared layout
- [ ] 04-02: Dashboard, Personalize (lesson + cluster + KB selection), Workspace (version timeline + refine + feedback + export), Settings, My Classes, Lesson Library, KB Browser, Print/Export pages
...
| 2. Core Data API | 0/3 | Not started | - |
| 3. Adaptation Engine | 0/3 | Not started | - |
| 4. React Frontend | 0/2 | Not started | - |
